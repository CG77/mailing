...
class OS extends AbstractGraph {

    public function __construct( Campaign $item ) {
        parent::__construct( "ezcGraphPieChart" );
        $result  = Collection::fetchBrowserStats( $item->attribute( 'id' ), 'os_name' );
        $dataSet = array();
        foreach( $result['result'] as $info ) {
            $dataSet[$info['type']]++;
        }
        $this->_graph->title           = ezpI18n::tr( 'extension/ezmailing/text', 'OS Repartition' );
